...
- [x] **P0-1 RLS is app-layer only on neon-http** — `packages/db/index.ts:1` uses `neon-http`; `setRlsContext()` (`lib/trpc/server.ts:133`) `SET LOCAL` silently no-ops. `FORCE RLS` (`0006_enable_rls.sql`) is defense-in-depth only for TCP/WebSocket. Primary is `entityScopingMiddleware` (`server.ts:249`). Documented `DATABASE.md:1657` as `USE_RLS` deferred. *Decision needed:* move to `neon-serverless Pool` **or** sign exception. **Critical / Security** — fixed `packages/db/index.ts` Pool opt-in (`USE_RLS=true` → `neon-serverless` + `ws`, else `neon-http`), `server.ts:133` warn, `schema/security.ts:51` param-bound, `DATABASE.md §14` + `ADR-RLS-POOL.md` + `.env.example`, verified `USE_RLS=true` Pool log + `neon-http` warn 2026-09-04
- [x] **P0-2 Hardcoded `GMD` display** — 14 sites ignore `ctx.entityCurrency` (`server.ts:305`). USD/XOF invoices show wrong symbol. `journal.ts:459,591`, `ap.ts:340,558`, `ar.ts:778`, `banking.ts:390`. **Critical / Financial** — fixed all `GMD ${` displays → `${currency} ${` across 14 routers (ap, ar, banking, journal + expenses, payroll, reconciliation, reports, transactions, bills, customers, etc.), added `const currency = (ctx as {entityCurrency?:string|null}).entityCurrency ?? "GMD"` per procedure, verified `GMD ${` count 0, `idor` still 69 passed 2026-09-04
...
> `AGENTS.md` — 5 surfaces only: Command Center, Activity Hub, Financial Pulse, Ledger, Operations. **Approved 2026-09-04: 6th surface `People & Assets` to surface hidden payroll/inventory/assets/budget pipelines.**
