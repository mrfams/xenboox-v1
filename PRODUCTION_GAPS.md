...
- [ ] **P0-3 Deletes missing `entityId` scope** — `delete().where(eq(id))` can delete cross-entity on race, even though prior check exists. `ar.ts:588,635,679` (`customers`, `salesInvoices`, `paymentsAr`), `ap.ts:1563,1707` (`suppliers`, `paymentsAp`). Fix: `where(and(eq(id), eq(entityId, ctx.entityId!)))`. **High / Security**
