...
describe("AI Preferences Summary", () => {
  ...
  describe("Status summary", () => {
    it("counts enabled preferences correctly", () => {
      const prefs = { autoReconcile: true, autoCategorize: false, aiAlerts: true, dailyDigest: false }
      const enabled = Object.values(prefs).filter(Boolean).length
      expect(enabled).toBe(2)
    })

    it("shows all active when all enabled", () => {
      const prefs = { autoReconcile: true, autoCategorize: true, aiAlerts: true, dailyDigest: true }
      const enabled = Object.values(prefs).filter(Boolean).length
    ...
    it("shows all disabled when none enabled", () => {
      const prefs = { autoReconcile: false, autoCategorize: false, aiAlerts: false, dailyDigest: false }
      const enabled = Object.values(prefs).filter(Boolean).length
  ...
  describe("Persistence", () => {
    ...
    it("merges stored prefs with defaults", () => {
      ...
      const defaults = { autoReconcile: true, autoCategorize: true, aiAlerts: true, dailyDigest: true }
  ...
  describe("Accessibility", () => {
    ...
    it("each toggle has label and description", () => {
      const toggles = [
        { label: "Auto-Reconciliation", desc: "AI matches bank transactions to journal entries" },
        { label: "Smart Categorization", desc: "AI suggests expense categories" },
        { label: "Anomaly Alerts", desc: "AI notifies of unusual transactions" },
