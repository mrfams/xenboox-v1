...
describe("Export/Import Settings", () => {
  describe("Exported settings structure", () => {
    ...
    it("has exportedAt timestamp", () => {
      ...
      expect(new Date(mockExport.exportedAt).toISOString()).toBe(mockExport.exportedAt)
    })

    it("has aiPreferences or null", () => {
      expect(mockExport.aiPreferences !== null || mockExport.aiPreferences === null).toBe(true)
    })
  ...
  describe("Import preview", () => {
    ...
    it("shows AI preferences count", () => {
      const prefs = { autoReconcile: true, autoCategorize: true, aiAlerts: false, dailyDigest: true }
      const enabled = Object.values(prefs).filter(Boolean).length
  ...
  describe("Export items", () => {
    const items = [
      ...
      { label: "Notification Preferences", available: false, note: "Server-side" },
    ]
