...
describe("Sidebar Onboarding Progress Indicator", () => {
  ...
  describe("Completed steps tracking", () => {
    it("step 0 has 0 completed steps", () => {
      const steps = ["welcome", "chart-of-accounts", "bank-connection", "team", "ai-preferences"]
      const stepIndex = 0
  ...
  describe("UI elements", () => {
    ...
    it("shows percentage in monospace font", () => {
      const html = 'className="ml-auto text-[10px] font-mono text-muted-foreground"'
      expect(html).toContain("font-mono")
    })

    it("shows progress bar with primary color", () => {
      const html = 'className="h-full rounded-full bg-primary transition-all duration-500 ease-out"'
    ...
    it("current step shows pulsing dot", () => {
      const html = 'className="h-3 w-3 shrink-0 rounded-full border-2 border-primary bg-primary/20"'
