...
describe("Mobile Bottom Nav with Onboarding Progress", () => {
  ...
  describe("Progress indicator", () => {
    ...
    it("current step dot is wider", () => {
      const html = 'className="h-1.5 w-1.5 rounded-full transition-all ... bg-primary w-3"'
      expect(html).toContain("w-3")
    })

    it("completed dots are solid", () => {
      const html = 'className="h-1.5 w-1.5 rounded-full transition-all ... bg-primary"'
      expect(html).toContain("bg-primary")
    })

    it("future dots are faded", () => {
      const html = 'className="h-1.5 w-1.5 rounded-full transition-all ... bg-primary/20"'
