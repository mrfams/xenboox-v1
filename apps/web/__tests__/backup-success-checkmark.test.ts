...
describe("Backup Success Checkmark Animation", () => {
  describe("CheckCircle2 icon", () => {
    ...
    it("CheckCircle2 has scale-in animation", () => {
      const classes = "h-4 w-4 shrink-0 text-emerald-500 animate-[scaleIn_0.3s_ease-out]"
  ...
  describe("Three-state icon display", () => {
    it("shows Loader2 spinner when pending", () => {
      ...
      const icon = isPending ? "Loader2" : justCompleted ? "CheckCircle2" : "ShieldCheck"
  ...
  describe("Text changes", () => {
    it("shows 'Creating backup...' when pending", () => {
      const isPending = true
      const justCompleted = false
      const text = isPending ? "Creating backup..." : justCompleted ? "Backup saved!" : "A backup will be created automatically."
