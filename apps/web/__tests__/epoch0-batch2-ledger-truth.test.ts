...
describe("N17 banking manual-connection honesty", () => {
  it("source code contains no demo transaction generator", async () => {
    ...
    const src = fs.readFileSync("apps/web/server/routers/banking.ts", "utf8");
...
describe("N12 approval-time re-validation", () => {
  it("approvals.resolve runs TrustGuard before the status flip", async () => {
    const fs = await import("node:fs");
    const src = fs.readFileSync("apps/web/server/routers/approvals.ts", "utf8");
...
describe("N16 withTimeout cooperative cancellation", () => {
  it("fires onTimeout BEFORE rejecting with TimeoutError", async () => {
    const { withTimeout, TimeoutError } = await import("@xenboox/agents");
    let fired = false;
    await expect(
      withTimeout(
  ...
  it("does not fire onTimeout on success", async () => {
    const { withTimeout } = await import("@xenboox/agents");
    let fired = false;
    const result = await withTimeout(async () => "ok", 500, "test-op", {
      onTimeout: () => { fired = true; },
    });
  ...
  it("close pipeline checks the abort flag before each write stage", async () => {
    const fs = await import("node:fs");
    const src = fs.readFileSync("packages/agents/core/close-pipeline.ts", "utf8");
