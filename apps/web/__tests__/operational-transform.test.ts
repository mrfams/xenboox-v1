...
describe("Operational Transform for Settings Sync", () => {
  ...
  describe("Operation transform — same path", () => {
    it("pending wins if it's newer", () => {
      ...
      const pendingWins =
        new Date(pending.timestamp) > new Date(concurrent.timestamp);
    ...
    it("concurrent wins if it's newer", () => {
      ...
      const concurrentWins =
        new Date(concurrent.timestamp) > new Date(pending.timestamp);
  ...
  describe("Extract operations from diff", () => {
    ...
    it("detects changed fields", () => {
      ...
      const changed = Object.keys(newSettings).filter(
        (k) =>
          JSON.stringify((oldSettings as Record<string, unknown>)[k]) !==
          JSON.stringify((newSettings as Record<string, unknown>)[k]),
      );
    ...
    it("detects deleted fields", () => {
      ...
      const deleted = Object.keys(oldSettings).filter(
        (k) => !(k in newSettings),
      );
  ...
  describe("Optimistic locking", () => {
    it("server rejects if baseUpdatedAt mismatches", () => {
      const baseUpdatedAt: string = "2026-08-20T10:00:00Z";
      const serverUpdatedAt: string = "2026-08-20T10:05:00Z";
