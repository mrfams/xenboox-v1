...
const backupSectionSrc = readFileSync(
  resolve(__dirname, "../components/settings/backup-section.tsx"),
  "utf-8",
);
...
describe("Settings page sound integration", () => {
  it("Backup section imports SoundPreferences", () => {
    expect(backupSectionSrc).toContain(
      'import { SoundPreferences } from "@/components/settings/sound-preferences"',
    );
  });

  it("Backup section renders SoundPreferences", () => {
    expect(backupSectionSrc).toContain("<SoundPreferences />");
  });
