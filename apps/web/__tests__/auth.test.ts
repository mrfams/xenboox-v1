...
describe("Auth Router", () => {
  ...
  describe("register", () => {
    ...
    it("should create user, org, entity, and access on success", async () => {
      ...
      vi.mocked(db.insert).mockImplementation(() => {
        ...
        else if (callCount === 3) chain.returning.mockResolvedValue([mockEntity])
  ...
  describe("resetPassword", () => {
    it("should reject invalid token", async () => {
      ...
      await expect(
        caller.auth.resetPassword({ token: "bad-token", newPassword: "newpassword123" })
    ...
    it("should reject expired token", async () => {
      ...
      await expect(
        caller.auth.resetPassword({ token: "valid-token", newPassword: "newpassword123" })
