...
export const chatRouter = router({
  ...
  searchConversations: rlsProtectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const scopedMessageMatches = messageMatches.filter((m) =>
        entityConvIdSet.has(m.conversationId),
      );

      // Get unique conversation IDs from message matches
      const messageConvIds = [
        ...new Set(scopedMessageMatches.map((m) => m.conversationId)),
      ];

      // Fetch those conversations
      const convFromMessages =
        messageConvIds.length > 0
      ...
      for (const msg of scopedMessageMatches) {
        if (!allConvIds.has(msg.conversationId)) {
          ...
          const conv = convFromMessages.find(
          ...
          if (conv) {
            ...
            const end = Math.min(
              content.length,
              matchIndex + input.query.length + 40,
            );
            const snippet =
              (start > 0 ? "..." : "") +
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (!forkPointMessage) {
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Fork point message not found",
          });
        }
        ...
        const [newConversation] = await db
          ...
          .values({
            ...
            lastMessageAt:
              messagesToFork[messagesToFork.length - 1]?.createdAt ??
              new Date(),
          })
          .returning();
            ...
            await db.insert(chatAttachments).values(
              relevantAttachments.map((att) => ({
                ...
                messageId: att.messageId
                  ? (messageIdMap.get(att.messageId) ?? att.messageId)
                  : null,
    ...
    .query(async ({ ctx, input }) => {
      ...
      const shareUsers =
        shareUserIds.length > 0
          ? await db.query.users.findMany({
              where: or(...shareUserIds.map((id) => eq(users.id, id))),
            })
          : [];
      ...
      return shares.map((share) => ({
        ...
        sharedWithUser: share.sharedWithUserId
          ? userMap.get(share.sharedWithUserId)
          : null,
      }));
    }),
