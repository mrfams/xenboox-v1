...
Keep it professional but conversational. Use specific numbers. Don't be generic.`;
  ...
  getAiBriefing: rlsProtectedProcedure.query(async ({ ctx }) => {
    const { entityId } = ctx;
    const now = new Date();
...
If everything is good, say so briefly and mention one thing to watch.`;
  ...
  getAiForecast: rlsProtectedProcedure.query(async ({ ctx }) => {
    const entityId = ctx.entityId!;
    const entityName = ctx.entityName ?? "your business";
    const currency = ctx.currency ?? "GMD";
    const now = new Date();
...
- Return ONLY valid JSON, no markdown`;
    ...
    try {
      ...
      const projectedCashFlow = (forecast.projectedRevenue ?? []).map(
        (rev: number, i: number) =>
          rev - (forecast.projectedExpenses?.[i] ?? 0),
      );
