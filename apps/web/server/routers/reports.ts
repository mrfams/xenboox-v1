  ...
  getProfitAndLoss: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        const allRows: AccountRow[] = Array.from(accountMap.values()).map((r) => ({
          ...r,
          balance: r.debit - r.credit,
        }))

        const revenueAccounts = allRows.filter(
          (a) => a.type === "revenue"
        )
        const expenseAccounts = allRows.filter(
          (a) => a.type === "expense"
        )
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to generate P&L report" })
      }
    }),
  ...
  getBalanceSheet: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        const totalAssets = assetAccounts.reduce((s, a) => s + a.displayAmount, 0)
        const totalLiabilities = liabilityAccounts.reduce((s, a) => s + a.displayAmount, 0)
        const totalEquity = equityAccounts.reduce((s, a) => s + a.displayAmount, 0)
        ...
        if (error instanceof TRPCError) throw error
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to generate balance sheet" })
      }
    }),
