import { z } from "zod"
import { router, rlsProtectedProcedure, adminProcedure } from "@/lib/trpc/server"
import { db } from "@/lib/db"
import { eq, and, desc, or } from "drizzle-orm"
import { notifications, notificationTypeEnum, notificationPriorityEnum, notificationStatusEnum } from "@xenboox/db/schema/notifications"
...
export const notificationsRouter = router({
  ...
  unreadCount: rlsProtectedProcedure
    .query(async ({ ctx }) => {
  ...
  markAllAsRead: rlsProtectedProcedure
    .mutation(async ({ ctx }) => {
      await db.update(notifications)
        .set({ read: true })
        .where(
  ...
  create: adminProcedure
    ...
    .mutation(async ({ input }) => {
      const userId = input.userId || (await db.query.userEntityAccess.findFirst({
  ...
  admin: adminProcedure
    ...
    .query(async ({ input }) => {
      const conditions = []
      if (input.entityId) conditions.push(eq(notifications.entityId, input.entityId))
      if (input.type) conditions.push(eq(notifications.type, input.type))
      if (input.priority) conditions.push(eq(notifications.priority, input.priority))
