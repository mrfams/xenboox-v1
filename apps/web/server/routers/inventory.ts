  ...
  createWarehouse: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (!wh) {
          throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to create warehouse" })
        }
        ...
        await db.insert(auditLog).values({
          ...
          entityIdRef: wh.id,
          newValues: { name: input.name, location: input.location, managerName: input.managerName },
        })
  ...
  createItem: protectedProcedure
    .input(
      z.object({
        ...
        costMethod: z.enum(["fifo", "lifo", "weighted_average"]).default("weighted_average"),
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (!item) {
          throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to create inventory item" })
        }
        ...
        await db.insert(auditLog).values({
          ...
          entityIdRef: item.id,
          newValues: { name: input.name, sku: input.sku, category: input.category, costMethod: input.costMethod, standardCost: input.standardCost, reorderLevel: input.reorderLevel },
        })
  ...
  createTransaction: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        return db.transaction(async (tx) => {
          ...
          if (txRecord) {
            await tx.insert(auditLog).values({
              ...
              newValues: { inventoryItemId: input.inventoryItemId, type: input.type, quantity: input.quantity, unitCost: input.unitCost, totalCost: totalCost.toFixed(2) },
            })
          }
        ...
        if (error instanceof TRPCError) throw error
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to create inventory transaction" })
      }
    }),
