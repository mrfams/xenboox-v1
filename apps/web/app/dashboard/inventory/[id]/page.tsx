...
export default function InventoryItemDetailPage() {
  ...
  const transactions = (item as Record<string, unknown>).transactions as Array<{
    id: string
    type: string
    quantity: number
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground">{item.description ?? item.sku}</p>
        </div>
      ...
      {lowStock && (
        <div className="flex items-center gap-2 rounded-lg border border-amber-300 bg-amber-50 dark:border-amber-700 dark:bg-amber-900/20 p-4 text-sm text-amber-800 dark:text-amber-300">
          ...
          Low stock warning: quantity on hand ({Number(item.quantityOnHand)}) is below reorder level ({Number(item.reorderLevel)}).
        </div>
      )}
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        ...
        <div className="space-y-4 rounded-lg border bg-card p-6">
          <h3 className="text-sm font-medium text-muted-foreground">Stock & Costing</h3>
          <dl className="space-y-3">
            <div className="flex justify-between">
              <dt className="text-sm text-muted-foreground">Standard Cost</dt>
              <dd className="text-sm font-mono font-medium">{formatCurrency(Number(item.standardCost))}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-sm text-muted-foreground">Quantity On Hand</dt>
              <dd className="text-sm font-mono font-medium">{Number(item.quantityOnHand)}</dd>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              <dt className="text-sm text-muted-foreground">Reorder Quantity</dt>
              <dd className="text-sm font-mono">{Number(item.reorderQuantity)}</dd>
            </div>
          </dl>
        </div>
      </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="py-3 px-4 text-right text-xs font-medium text-muted-foreground">Quantity</th>
                  <th className="py-3 px-4 text-left text-xs font-medium text-muted-foreground">Notes</th>
                </tr>
              </thead>
              <tbody>
                {transactions.map((tx) => (
                  <tr key={tx.id} className="border-b hover:bg-muted/50">
                    <td className="py-3 px-4 text-sm">{formatDate(tx.referenceDate)}</td>
                    <td className="py-3 px-4 text-sm">
                      <Badge variant="secondary">{tx.type}</Badge>
                    </td>
                    <td className="py-3 px-4 text-sm text-right font-mono">{tx.quantity}</td>
                    <td className="py-3 px-4 text-sm text-muted-foreground">{tx.notes ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
