...
export default function ImprestFloatDetailPage() {
  ...
  const { data: float, isLoading } = trpc.cash.getImprestFloatById.useQuery({ id })
  ...
  const receipts = (float as Record<string, unknown>).receipts as Array<{
    id: string
    description: string
    amount: string
    receiptDate: string
        ...
        <div className="space-y-4 rounded-lg border bg-card p-6">
          ...
          <dl className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <dd className="text-sm">{float.settleByDate ? formatDate(float.settleByDate) : "—"}</dd>
            </div>
          </dl>
        </div>
        ...
        <div className="space-y-4 rounded-lg border bg-card p-6">
          ...
          <dl className="space-y-3">
            <div className="flex justify-between">
              ...
              <dd className="text-sm font-mono font-medium">{formatCurrency(Number(float.amount))}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-sm text-muted-foreground">Remaining</dt>
              <dd className="text-sm font-mono font-medium">{formatCurrency(Number(float.remainingBalance))}</dd>
            </div>
            <div className="flex justify-between">
              ...
              <tbody>
                {receipts.map((r) => (
                  <tr key={r.id} className="border-b hover:bg-muted/50">
                    <td className="py-3 px-4 text-sm">{formatDate(r.receiptDate)}</td>
