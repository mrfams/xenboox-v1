...
export default function SupplierDetailPage() {
  ...
  const supplierPayments = payments?.filter((p) => supplierInvoiceIds.has(p.invoiceApId)) ?? []
    ...
    <DetailShell
      ...
      tabs={[
              ...
              <div className="space-y-4 rounded-lg border bg-card p-6">
                ...
                <dl className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <dd className="text-sm">{supplier.contactEmail ?? "—"}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm text-muted-foreground">Phone</dt>
                    <dd className="text-sm">{supplier.contactPhone ?? "—"}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm text-muted-foreground">Tax ID</dt>
                    <dd className="text-sm font-mono">{supplier.taxId ?? "—"}</dd>
                  </div>
                </dl>
              </div>
              <div className="space-y-4 rounded-lg border bg-card p-6">
                ...
                <dl className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <dd className="text-sm text-right max-w-[200px]">{supplier.address ?? "—"}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm text-muted-foreground">Payment Terms</dt>
                    <dd className="text-sm">{supplier.paymentTerms ?? "—"}</dd>
                  </div>
                  <div className="flex justify-between">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b bg-muted/50">
                    <th className="py-3 px-4 text-left text-xs font-medium text-muted-foreground">PO #</th>
        ...
        },
        {
          ...
          content: supplierInvoices.length === 0 ? (
            <EmptyState
                ...
                <tbody>
                  {supplierInvoices.map((inv) => (
                    <tr
                      key={inv.id}
                      className="border-b hover:bg-muted/50 cursor-pointer"
                      onClick={() => router.push(`/dashboard/ap/invoices/${inv.id}`)}
                    >
        ...
        },
        {
          ...
          content: supplierPayments.length === 0 ? (
            <EmptyState
