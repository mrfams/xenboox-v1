...
export default function TrialBalancePage() {
  const { data: periods, isLoading: periodsLoading } = trpc.reports.listPeriods.useQuery()
  const [selectedPeriod, setSelectedPeriod] = useState<string>("")

  const { data: trialBalance, isLoading } = trpc.journal.getTrialBalance.useQuery(
          ...
          <table className="w-full">
            ...
            <tbody>
              {trialBalance.accounts.map((row) => (
                <tr key={row.accountId} className="border-b last:border-0 hover:bg-muted/50">
                  ...
                  <td className="px-4 py-3 text-sm text-muted-foreground capitalize">{row.type}</td>
            ...
            <tfoot>
              <tr className="border-t-2 font-bold bg-muted/30">
                ...
                <td className={`px-4 py-3 text-sm text-right font-mono ${
                  trialBalance.isBalanced ? "text-emerald-600 dark:text-emerald-400" : "text-destructive"
                }`}>
