...
type AdminJobListing = JobListing & {
  applications: number;
};

const initialJobs: AdminJobListing[] = jobListings.map((job) => ({
  ...job,
  applications: Math.floor(Math.random() * 100) + 10,
}));

export default function CareersAdminPage() {
  const [jobs, setJobs] = useState<AdminJobListing[]>(initialJobs);
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedDepartment, setSelectedDepartment] = useState("All");
  const [selectedStatus, setSelectedStatus] = useState("All");
  const [showEditor, setShowEditor] = useState(false);
  const [editingJob, setEditingJob] = useState<AdminJobListing | null>(null);
  ...
  const handleEditJob = (job: AdminJobListing) => {
    setEditingJob(job);
    setFormData({
      title: job.title,
  ...
  const handleToggleActive = (jobId: string) => {
    setJobs(
      jobs.map((j) => (j.id === jobId ? { ...j, isActive: !j.isActive } : j)),
    );
  };
