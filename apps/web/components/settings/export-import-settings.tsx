...
export function ExportImportSettings({ onViewBackup }: { onViewBackup?: () => void } = {}) {
  ...
  const handleImportApply = () => {
    ...
    if (importPreview.onboarding.currentStep) {
      localStorage.setItem(KEYS.onboardingStep, importPreview.onboarding.currentStep)
    } else {
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-sm text-muted-foreground">
          Export your preferences to a JSON file, or import settings from another device.
        </p>

        {/* Export Summary */}
        <div className="rounded-lg border bg-muted/30 p-3 space-y-2">
          <p className="text-xs font-medium text-muted-foreground">Will export:</p>
          <div className="space-y-1">
            <ExportItem label="AI Preferences" available={true} />
            <ExportItem label="Onboarding Status" available={true} />
            <ExportItem label="Notification Preferences" available={false} note="Server-side" />
          </div>
        </div>
        ...
        <div className="group relative flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950 dark:text-emerald-300">
          ...
          <span>A backup will be created automatically before importing.</span>            <div className="absolute bottom-full left-0 mb-2 hidden w-72 rounded-lg border bg-popover p-3 text-xs text-popover-foreground shadow-md group-hover:block z-50">
              ...
              <div className="space-y-3 py-2">
                <div className="rounded-lg border bg-muted/30 p-3 space-y-2">
                  <p className="text-xs font-medium text-muted-foreground">
                    Imported from: {new Date(importPreview.exportedAt).toLocaleDateString()}
                  </p>
                  <div className="space-y-1">
                    {importPreview.aiPreferences && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-xs text-muted-foreground ml-auto">
                          {Object.values(importPreview.aiPreferences).filter(Boolean).length}/4 enabled
                        </span>
                      </div>
                    )}
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-xs text-muted-foreground ml-auto">
                        {importPreview.onboarding.completed ? "Completed" : "In progress"}
                      </span>
                    </div>
                  </div>
                </div>
...
function ExportItem({
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      {note && (
        <span className="text-muted-foreground ml-auto">({note})</span>
      )}
    </div>
  )
}
