...
import { getStrategyLabel } from "@/lib/merge-strategies"
...
const STRATEGY_COLORS: Record<string, string> = {
  "last-write-wins": "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400",
  "local-wins": "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400",
  "remote-wins": "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400",
  "deep-merge": "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400",
...
export function ConflictResolutionHistory() {
  ...
  const { data: history, isLoading } = trpc.settings.getConflictHistory.useQuery(
    { limit: 10 },
    { enabled: isExpanded }
  )
        ...
        <CardContent className="space-y-3">
          ...
          {isLoading ? (
            ...
            <div className="rounded-lg border bg-muted/30 p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">No conflicts resolved yet</p>
              <p className="text-xs text-muted-foreground mt-1">
                Conflicts appear when settings are modified on multiple devices simultaneously.
              </p>
            </div>
          ) : (
            <div className="space-y-2">
...
function ConflictHistoryEntry({
  ...
  const strategyColor = STRATEGY_COLORS[entry.strategy] || "bg-muted text-muted-foreground"
  ...
  return (
    <div className="rounded-lg border bg-muted/30 overflow-hidden">
      ...
      >
        {/* Strategy badge */}
        <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium", strategyColor)}>
        ...
        <div className="flex items-center gap-1 text-xs text-muted-foreground">
          ...
          <span>{entry.conflictCount} conflict{entry.conflictCount !== 1 ? "s" : ""}</span>
        </div>
        ...
        <div className="border-t p-3 space-y-3">
          ...
          {entry.conflicts && entry.conflicts.length > 0 && (
            <div className="space-y-1">
              <p className="text-xs font-medium text-muted-foreground">Conflicting fields:</p>
