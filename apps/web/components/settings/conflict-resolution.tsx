...
export function ConflictResolution({
  ...
  const [selectedStrategy, setSelectedStrategy] =
    useState<MergeStrategy>("deep-merge");
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-lg border border-amber-200 bg-white p-3 dark:border-amber-800 dark:bg-amber-900/50">
          <p className="text-sm text-amber-700 dark:text-amber-300">
            Settings were modified on <strong>another device</strong> while you
            were offline.
          </p>
          <div className="mt-2 flex items-center gap-4 text-xs text-amber-600 dark:text-amber-400">
            <div className="flex items-center gap-1">
              ...
              <span>
                Local: {localUpdatedAt ? formatTime(localUpdatedAt) : "Unknown"}
              </span>
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              <span>
                Remote:{" "}
                {remoteUpdatedAt ? formatTime(remoteUpdatedAt) : "Unknown"}
              </span>
            </div>
          </div>
        </div>
        ...
        <p className="text-sm text-muted-foreground">
          <strong>{conflicts.length}</strong> setting
          ...
          >
            <AlertDialogTrigger asChild>
              <Button
                ...
                disabled={
                  selectedStrategy === "manual" &&
                  Object.keys(manualResolutions).length === 0
                }
              >
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  You are about to resolve {conflicts.length} conflict
                  {conflicts.length !== 1 ? "s" : ""} using{" "}
                  <strong>{getStrategyLabel(selectedStrategy)}</strong>.
                  {selectedStrategy === "manual" && (
                    <span>
                      {" "}
                      You have resolved {
                        Object.keys(manualResolutions).length
                      }{" "}
                      of {conflicts.length} conflicts.
                    </span>
                  )}
                </AlertDialogDescription>
              </AlertDialogHeader>
        ...
        <div className="space-y-2 border-t p-2">
          ...
          <div className="flex gap-2">
            <Button
              size="sm"
              variant={
                resolvedValue === conflict.localValue ? "default" : "outline"
              }
            ...
            <Button
              size="sm"
              variant={
                resolvedValue === conflict.remoteValue ? "default" : "outline"
              }
