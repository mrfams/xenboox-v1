...
export function LoginForm({
  ...
  const searchCallbackUrl = sp.get("callbackUrl");
  const searchExpired = sp.get("expired") === "1";
  ...
  // Build-time flag: hide the Google button entirely when OAuth credentials
  // are not configured — otherwise clicking it redirects to
  // /login?error=Configuration with no explanation.
  ...
  useEffect(() => {
    // NextAuth redirects failed OAuth/Config back to /login?error=...
    // Translate to a plain-English message instead of a dead redirect.
    const authError = sp.get("error");
    if (authError) {
      const messages: Record<string, string> = {
        Configuration:
          "Google sign-in is not available right now. Please use email and password instead.",
