...
export function transformOperation(
  ...
  if (
    concurrent.path.startsWith(pending.path + ".") ||
...
function transformSamePath(
  ...
  return {
    ...
    wasModified: false,
    reason:
      "Concurrent operation is newer, pending operation should be retried",
  };
}
...
export function wouldConflict(
  ...
  conflictingOp?: SettingsOperation;
  reason?: string;
} {
  for (const pending of pendingOps) {
    ...
    if (
      operation.path.startsWith(pending.path + ".") ||
