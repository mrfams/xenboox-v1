...
//
// Full country list for the self-service tax flow: users pick their country,
// then install or build the taxes that jurisdiction requires. Flags are
// derived from the 2-letter ISO code via regional-indicator symbols, so the
...
/** Regional-indicator flag derived from an ISO 3166-1 alpha-2 code. */
export function countryFlag(code: string): string {
  const upper = code.toUpperCase();
  if (!/^[A-Z]{2}$/.test(upper)) return "🌐";
  return String.fromCodePoint(
    ...Array.from(upper, (c) => 0x1f1e6 + c.charCodeAt(0) - 0x41),
  );
}
...
const COUNTRY_LOOKUP = new Map(COUNTRIES.map((c) => [c.code, c]));

/** Looks up a country by ISO code; undefined if not in the registry. */
export function getCountry(code: string): CountryEntry | undefined {
  return COUNTRY_LOOKUP.get(code.toUpperCase());
}

/** Country display name with flag, e.g. "🇬🇲 Gambia (GM)". */
export function countryLabel(code: string): string {
  const c = getCountry(code);
  return c ? `${countryFlag(code)} ${c.name} (${code})` : `${code}`;
}
