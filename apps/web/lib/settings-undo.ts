...
export function showUndoToast(operationName: string) {
  ...
  toast(`${operationName} completed. A backup was saved automatically.`, {
...
async function fetchLatestVersionForTracking(): Promise<{
  id: string;
  label: string | null;
} | null> {
  try {
...
async function restoreLatestBackup() {
  try {
    ...
    if (restoreData?.result?.data) {
      toast.success(
        `Restored from ${latestVersion.label || `version ${latestVersion.version}`}`,
      );
