...
export type MergeStrategy =
  "last-write-wins" | "local-wins" | "remote-wins" | "deep-merge" | "manual";
...
export function detectConflicts(
  ...
  const localChanged =
    ...
    JSON.stringify(getLastSyncedSettings(lastSyncedAt));
...
export function getStrategyDescription(strategy: MergeStrategy): string {
  const descriptions: Record<MergeStrategy, string> = {
    "last-write-wins":
      "The most recently saved change wins. Remote changes override local.",
    "local-wins":
      "Keep your local changes. Remote changes are discarded for conflicts.",
    "remote-wins":
      "Accept all remote changes. Your local changes are discarded for conflicts.",
    "deep-merge":
      "Merge settings field by field. Non-conflicting fields are combined.",
