...
**Agent:** Buffy (Autonomous Engineer)
**Files Created:** `apps/web/lib/trpc/rate-limit-headers.ts`, `apps/web/__tests__/rate-limit-headers.test.ts` (5 tests), `apps/mobile/app/(auth)/forgot-password.tsx`, `apps/mobile/app/(auth)/reset-password.tsx`, `apps/mobile/app/(auth)/verify-email.tsx`, `docs/PRODUCTION_CREDENTIALS.md`
**Files Modified:** `apps/web/app/api/trpc/[trpc]/route.ts` (responseMeta), `apps/web/server/routers/organization.ts` (listUserEntities + currency), `apps/web/lib/entity-context.tsx` (entityCurrency), 3 create dialogs (currency default), `apps/web/app/api/chat/stream/route.ts` + `seed-demo/route.ts` (maxDuration), `apps/web/__tests__/a11y-static.test.ts` (prettier-tolerant regex), `apps/mobile/app/(auth)/login.tsx` (forgot link), removed `apps/mobile/lib/api.ts` + `constants/theme.ts` (dead code), `ROADTOPRODUCTION.md` §6.3/7.5/11.8/17.6/19.4

**What was done (autoplan):**

1. **§19.4 rate-limit headers on tRPC** — the edge middleware already set them for /api/\*; the tRPC layer was missing them. Rate-limit middleware now stores `ctx.rateLimitInfo`; `rate-limit-responseMeta` (`lib/trpc/rate-limit-headers.ts`, pure + unit-tested) emits `X-RateLimit-Limit/Remaining/Reset` on every response and `Retry-After` on 429.
2. **§6.3 hardcoded GMD currency** — `listUserEntities` now returns each entity's `currency`; `entity-context` exposes `entityCurrency`; the create-invoice/bill/bank-account dialogs default to the active entity's currency (GMD fallback).
3. **§7.5 mobile auth gaps** — added forgot-password, reset-password, and verify-email screens wired to the existing server procedures (rate-limited, no account enumeration) + login link. Removed dead `lib/api.ts` and `constants/theme.ts` (§11.8 — NativeWind tailwind config already carries the tokens).
4. **§17.6 maxDuration** — chat stream + seed-demo routes set `maxDuration = 300`; document-processing/month-end close run on Trigger.dev (no function ceiling). Verified §8.4 (agent timeouts/retries/circuit breakers) already implemented in `packages/models/router.ts`.
5. **Credentials doc** — `docs/PRODUCTION_CREDENTIALS.md` consolidates every MOCK\_\*/unset env var the user must replace before launch (R2 trio, OTel collector, BetterStack, budgets, autonomy, etc.) with exact env-var names and actions.
6. **Fixed a false-failure** — the a11y static test broke because prettier reformats `<main id>` to multiline; made the regex whitespace-tolerant.

**Verification:** web typecheck clean, build green, 959 passed / 15 pre-existing DB-env failures / 19 skipped (+5 new). Mobile tsc: my files add only the pre-existing router-type cascade class (baseline 2083–2169 errors); new auth screens compile, href types regenerated by expo.

---

### [2026-08-15] — Mobile hardening: offline read cache + skeletons + error/retry states (§7.1, §7.3, §7.4)

**Agent:** Buffy (Autonomous Engineer)
**Files Created:** `apps/mobile/components/ui/skeleton.tsx`, `apps/mobile/lib/read-cache-link.ts`
**Files Modified:** `apps/mobile/lib/offline-storage.ts` (read_cache table + typed callbacks via `expo-sqlite/legacy`), `apps/mobile/lib/trpc.ts` (read-cache link wired), `apps/mobile/lib/sync-service.ts` (cache sweep + missing `saveTransaction` import), 6 detail screens (`[id].tsx` — skeleton/error/retry), 5 list screens + 2 tab screens (skeletons), `ROADTOPRODUCTION.md` §7.1/7.3/7.4

**What was done (autoplan):**

1. **Offline read-through cache** — `read_cache` SQLite table (entity-scoped keys, 24h TTL) + `read-cache-link.ts` tRPC link: caches successful query results, and on network failure serves the cached copy (stale-while-offline). Swept by the sync service on reconnect. The offline indicator now reflects real offline functionality — list/detail data renders with no connection.
2. **Skeletons** — `Skeleton` (animated pulse) + `CardSkeleton` in `components/ui/skeleton.tsx`; all 5 list screens, AR list, and both tab screens (`(tabs)/index.tsx`, `invoices.tsx`) now render skeletons instead of bare "Loading..." text.
3. **Detail screen error/retry** — all 6 detail screens (AR, AP, payroll, inventory, fixed-assets, journal) now use `isLoading` → skeleton, `isError` → `ErrorComponent` with retry, replacing the conflated `!data` check that showed "Loading..." forever on failure.
4. **Type hygiene** — typed every SQL callback in `offline-storage.ts` with `expo-sqlite/legacy` types (cleared the file's pre-existing implicit-any errors), fixed the missing `saveTransaction` import in `sync-service.ts` (a real runtime bug in the offline queue path) and the broken `NetInfo` named import.

**Verification:** mobile `tsc --noEmit` — my files contribute 0 errors (net count 2169 → 2140 vs clean baseline; remaining errors are the pre-existing web-router type cascade).
