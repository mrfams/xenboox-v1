ALTER TABLE "audit_log" ADD COLUMN "actor_type" text;--> statement-breakpoint
ALTER TABLE "audit_log" ADD COLUMN "agent_id" text;--> statement-breakpoint
...
ALTER TABLE "audit_log" ADD COLUMN "event_hash" text;--> statement-breakpoint
ALTER TABLE "audit_log" ADD COLUMN "payload_hash_input" text;--> statement-breakpoint
CREATE INDEX "audit_entity_seq" ON "audit_log" USING btree ("entity_id","seq");--> statement-breakpoint

-- ─── Tamper-Evident Audit Chain ───────────────────────────────────────────
--
-- Every audit_log row is chained to the previous row of the same entity via
-- SHA-256: event_hash = sha256(prev_hash || E'\n' || payload_text). The
-- canonical payload text is stored (payload_hash_input) so verification hashes
-- the EXACT text the trigger hashed — no cross-language re-serialization.
--
-- MUST stay in sync with apps/web/lib/audit/chain.ts (canonicalize) and
-- apps/web/lib/audit/backfill.ts (rowToChainPayload): same field list, same
-- key order (jsonb_build_object preserves insertion order; canonicalize sorts
-- keys alphabetically — both produce the same final text), same coercions
-- (to_char ISO-8601 UTC for timestamps, ::text for numeric so Postgres number
-- formatting can never drift from JS).
...
-- BEFORE INSERT: assign seq, link prev_hash, compute event_hash, and store the
-- exact hashed payload text. Serializes per-entity via a row lock on the
-- entity (entity_id is NOT NULL and references entities) so concurrent inserts
-- cannot double-assign a seq. The first event in an entity's chain links to
-- GENESIS_HASH = sha256('xenboox-audit-genesis') — matches
-- GENESIS_HASH in lib/audit/chain.ts.
...
-- ─── Immutability ─────────────────────────────────────────────────────────
-- audit_log is append-only. UPDATE and DELETE are blocked at the database
-- layer; a would-be tamperer must modify the trigger itself (which is exactly
-- the class of change the hash chain + external anchoring follow-up detects).
-- NOTE: this MUST be created AFTER the backfill above — the backfill is an
-- UPDATE on audit_log and would otherwise be blocked by its own immutability
-- trigger. Keep this ordering: chain trigger → backfill → immutability.
CREATE OR REPLACE FUNCTION audit_log_block_mutation() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'audit_log is append-only: UPDATE/DELETE is not permitted (event %)', OLD.id;
END;
$$;--> statement-breakpoint
