...
export const bankRuleMatchTypeEnum = pgEnum("bank_rule_match_type", [
...
export const bankAccounts = pgTable(
  ...
  (t) => [index("bank_accounts_entity").on(t.entityId)],
);
...
export const reconciliationsRelations = relations(
  ...
  ({ one, many }) => ({
    entity: one(entities, {
      fields: [reconciliations.entityId],
      references: [entities.id],
    }),
    bankAccount: one(bankAccounts, {
...
// ─── BANK RULES ─────────────────────────────────
//
// Auto-categorization rules for bank transactions. When a statement is
// imported or a transaction syncs in, rules are evaluated in priority
// order; the first match assigns the category and (optionally) GL account.
// Matching happens at ingestion time via the banking pipeline; this table
// is the entity-scoped source of truth for those rules.
...
export const bankRules = pgTable(
  "bank_rules",
  {
    ...
    entityId: entityId.references(() => entities.id, { onDelete: "cascade" }),
    name: text("name").notNull(),
    matchType: bankRuleMatchTypeEnum("match_type").notNull(),
  ...
  (t) => [
    index("bank_rules_entity").on(t.entityId),
    index("bank_rules_active").on(t.entityId, t.isActive),
  ],
);

export const bankRulesRelations = relations(bankRules, ({ one }) => ({
  entity: one(entities, {
    fields: [bankRules.entityId],
    references: [entities.id],
  }),
  glAccount: one(chartOfAccounts, {
    fields: [bankRules.glAccountId],
    references: [chartOfAccounts.id],
  }),
}));
