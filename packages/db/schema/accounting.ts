...
export const trialBalanceSnapshots = pgTable("trial_balance_snapshots", {
  id: uuidId(),
  entityId: entityId.references(() => entities.id, { onDelete: "cascade" }),
  periodId: uuid("period_id").notNull().references(() => fiscalPeriods.id),
  accountId: uuid("account_id").notNull().references(() => chartOfAccounts.id),
  debitTotal: numeric("debit_total", { precision: 15, scale: 2 }).notNull().default("0"),
  creditTotal: numeric("credit_total", { precision: 15, scale: 2 }).notNull().default("0"),
  balance: numeric("balance", { precision: 15, scale: 2 }).notNull().default("0"),
  generatedBy: text("generated_by").notNull(),
  generatedAt: timestamp("generated_at").notNull().defaultNow()
}, (t) => [
  uniqueIndex("tbs_entity_period_account").on(t.entityId, t.periodId, t.accountId)
])
