import { pgTable, uuid, text, integer, numeric, boolean, timestamp, jsonb, pgEnum, index, uniqueIndex } from "drizzle-orm/pg-core"
...
export const suppliers = pgTable("suppliers", {
  ...
  index("suppliers_entity").on(t.entityId)
])

export const suppliersRelations = relations(suppliers, ({ one, many }) => ({
  entity: one(entities, { fields: [suppliers.entityId], references: [entities.id] }),
...
export const purchaseOrdersRelations = relations(purchaseOrders, ({ one, many }) => ({
  entity: one(entities, { fields: [purchaseOrders.entityId], references: [entities.id] }),
...
export const poLines = pgTable("po_lines", {
  ...
  index("po_lines_order").on(t.purchaseOrderId)
])
...
export const poLinesRelations = relations(poLines, ({ one }) => ({
  ...
  account: one(chartOfAccounts, { fields: [poLines.accountId], references: [chartOfAccounts.id] })
}))

// ─── AP INVOICES ─────────────────────────────────

export const invoicesAp = pgTable("invoices_ap", {
  id: uuidId(),
  entityId: entityId.references(() => entities.id, { onDelete: "cascade" }),
  supplierId: uuid("supplier_id").notNull().references(() => suppliers.id),
...
export const invoicesApRelations = relations(invoicesAp, ({ one, many }) => ({
  entity: one(entities, { fields: [invoicesAp.entityId], references: [entities.id] }),
  supplier: one(suppliers, { fields: [invoicesAp.supplierId], references: [suppliers.id] }),
  purchaseOrder: one(purchaseOrders, { fields: [invoicesAp.purchaseOrderId], references: [purchaseOrders.id] }),
  journalEntry: one(journalEntries, { fields: [invoicesAp.journalEntryId], references: [journalEntries.id] }),
...
export const invoiceApLines = pgTable("invoice_ap_lines", {
  id: uuidId(),
  invoiceApId: uuid("invoice_ap_id").notNull().references(() => invoicesAp.id, { onDelete: "cascade" }),
  accountId: uuid("account_id").notNull().references(() => chartOfAccounts.id),
  description: text("description").notNull(),
  quantity: numeric("quantity", { precision: 10, scale: 2 }).notNull().default("1"),
  unitPrice: numeric("unit_price", { precision: 15, scale: 2 }).notNull(),
  ...
  index("ap_lines_invoice").on(t.invoiceApId)
])

export const invoiceApLinesRelations = relations(invoiceApLines, ({ one }) => ({
  invoiceAp: one(invoicesAp, { fields: [invoiceApLines.invoiceApId], references: [invoicesAp.id] }),
  account: one(chartOfAccounts, { fields: [invoiceApLines.accountId], references: [chartOfAccounts.id] })
}))

// ─── AP PAYMENTS ─────────────────────────────────

export const paymentsAp = pgTable("payments_ap", {
  id: uuidId(),
  entityId: entityId.references(() => entities.id, { onDelete: "cascade" }),
  invoiceApId: uuid("invoice_ap_id").notNull().references(() => invoicesAp.id),
  amount: numeric("amount", { precision: 15, scale: 2 }).notNull(),
...
export const paymentsApRelations = relations(paymentsAp, ({ one }) => ({
  entity: one(entities, { fields: [paymentsAp.entityId], references: [entities.id] }),
  invoiceAp: one(invoicesAp, { fields: [paymentsAp.invoiceApId], references: [invoicesAp.id] }),
  journalEntry: one(journalEntries, { fields: [paymentsAp.journalEntryId], references: [journalEntries.id] })
}))
...
export const customers = pgTable("customers", {
  ...
  index("customers_entity").on(t.entityId)
])
...
export const salesInvoicesRelations = relations(salesInvoices, ({ one, many }) => ({
  entity: one(entities, { fields: [salesInvoices.entityId], references: [entities.id] }),
  customer: one(customers, { fields: [salesInvoices.customerId], references: [customers.id] }),
  journalEntry: one(journalEntries, { fields: [salesInvoices.journalEntryId], references: [journalEntries.id] }),
...
export const salesInvoiceLines = pgTable("sales_invoice_lines", {
  ...
  index("ar_lines_invoice").on(t.salesInvoiceId)
])

export const salesInvoiceLinesRelations = relations(salesInvoiceLines, ({ one }) => ({
  salesInvoice: one(salesInvoices, { fields: [salesInvoiceLines.salesInvoiceId], references: [salesInvoices.id] }),
  account: one(chartOfAccounts, { fields: [salesInvoiceLines.accountId], references: [chartOfAccounts.id] })
}))

// ─── AR PAYMENTS ─────────────────────────────────

export const paymentsAr = pgTable("payments_ar", {
  id: uuidId(),
  entityId: entityId.references(() => entities.id, { onDelete: "cascade" }),
...
export const paymentsArRelations = relations(paymentsAr, ({ one }) => ({
  entity: one(entities, { fields: [paymentsAr.entityId], references: [entities.id] }),
  salesInvoice: one(salesInvoices, { fields: [paymentsAr.salesInvoiceId], references: [salesInvoices.id] }),
  journalEntry: one(journalEntries, { fields: [paymentsAr.journalEntryId], references: [journalEntries.id] })
}))
