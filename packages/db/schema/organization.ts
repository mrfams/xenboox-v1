import { pgTable, uuid, text, boolean, jsonb, pgEnum, index, uniqueIndex } from "drizzle-orm/pg-core"
...
export const entities = pgTable("entities", {
  ...
  index("idx_entities_organization").on(t.organizationId)
])
...
export const userEntityAccessRelations = relations(userEntityAccess, ({ one }) => ({
  user: one(users, { fields: [userEntityAccess.userId], references: [users.id] }),
  entity: one(entities, { fields: [userEntityAccess.entityId], references: [entities.id] }),
  grantor: one(users, { fields: [userEntityAccess.grantedBy], references: [users.id] })
}))
