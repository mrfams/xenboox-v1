...
async function seed4Months() {
  console.log("Seeding 4-month expansion (Mar-Jun 2026)...");
  ...
  function startOf(m: number) {
    return `2026-${String(m + 1).padStart(2, "0")}-01`;
  }
  function midOf(m: number) {
    return `2026-${String(m + 1).padStart(2, "0")}-15`;
  }
  function endOf(m: number) {
    return `2026-${String(m + 1).padStart(2, "0")}-28`;
  }
  ...
  console.log("4-month expansion complete!");
  console.log("  Added: Mar, Apr, May, Jun data across modules");
}
