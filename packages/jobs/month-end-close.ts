...
import { journalEntries, fiscalPeriods, journalEntryLines, chartOfAccounts } from "@xenboox/db/schema"
...
export const processMonthEndClose = task({
  ...
  run: async (payload: {
    ...
    if (period.status === "closed") {
      ...
      return { success: true, message: "Period already closed", periodId: period.id }
    }

    // 2. Validate all journal entries are posted
    const draftEntries = await db.select().from(journalEntries).where(
      and(
    ...
    if (draftEntries.length > 0) {
      throw new Error(`${draftEntries.length} draft entries must be posted before close`)
    }
...
async function runDepreciation(entityId: string, periodId: string) {
  const fixedAssets = await db.select().from(chartOfAccounts).where(
    and(
  ...
  for (const asset of fixedAssets) {
    ...
    const accumAccounts = await db.select().from(chartOfAccounts).where(
      and(
        eq(chartOfAccounts.entityId, entityId),
    ...
    const expenseAccounts = await db.select().from(chartOfAccounts).where(
      and(
        eq(chartOfAccounts.entityId, entityId),
