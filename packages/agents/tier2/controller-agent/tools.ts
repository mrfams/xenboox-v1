...
export async function validateEntryStructural(
  ...
  if (entry.entries.length < 2) {
    return { approved: false, rejectionReason: "Entry must have at least 2 lines", structuralFlags: ["min_lines"], qualitativeFlags: [], confidence: 0 }
  }
  ...
  if (entry.description.toLowerCase().includes("reversal") || entry.description.toLowerCase().includes("reverse")) {
...
async function getAccountBalanceFromGL(entityId: string, accountCode: string): Promise<number> {
  const account = await db.query.chartOfAccounts.findFirst({
    where: and(
      eq(chartOfAccounts.entityId, entityId),
...
export async function reconcileSubLedgers(entityId: string): Promise<SubLedgerStatus> {
