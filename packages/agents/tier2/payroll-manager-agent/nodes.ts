...
import { validatePayrollData, checkTaxCalculations } from "./tools"
...
export async function nodeProcessPayroll(state: PayrollManagerStateType) {
  const trace = await langfuse.span({ name: "payroll-manager-process", input: state.currentOperation?.input })

  const payrollData = state.currentOperation?.input?.payroll as any
  if (!payrollData) {
  ...
  const totalGross = payrollData.employees.reduce((s: number, e: any) => s + e.grossPay, 0)
  const totalDeductions = payrollData.employees.reduce((s: number, e: any) => s + e.taxWithheld + e.benefits, 0)
  const totalNet = payrollData.employees.reduce((s: number, e: any) => s + e.netPay, 0)
...
export async function nodeValidatePayroll(state: PayrollManagerStateType) {
  ...
  await trace.update({ output: { taxValid: taxCheck.valid, errors: taxCheck.errors } })
