...
export function validatePayrollData(data: PayrollData): ValidationResult {
  ...
  for (let i = 0; i < data.employees.length; i++) {
    ...
    if (emp.grossPay <= 0)
      errors.push(`Employee ${i + 1}: gross pay must be positive`);
    if (emp.netPay < 0)
      errors.push(`Employee ${i + 1}: net pay cannot be negative`);
    if (emp.taxWithheld < 0)
      errors.push(`Employee ${i + 1}: tax withheld cannot be negative`);
