...
import { classifyUserMessage, checkEscalation } from "../core/orchestrator";
...
describe("classifyUserMessage", () => {
  it("classifies close trigger messages (must start with close/month.end/period.end AND contain close/run/process)", () => {
    ...
    expect(classifyUserMessage("period end close for June")).toBe(
      "close_trigger",
    );
    expect(classifyUserMessage("month end close process")).toBe(
      "close_trigger",
    );
  });
  ...
  it("classifies payroll messages", () => {
    expect(classifyUserMessage("process payroll for June")).toBe(
      "process_payroll",
    );
  ...
  it("classifies cash/treasury messages", () => {
    expect(classifyUserMessage("bank reconciliation status")).toBe(
      "cash_position",
    );
    expect(classifyUserMessage("our cash position is low")).toBe(
      "cash_position",
    );
  });
  ...
  it("classifies inventory messages (must not contain 'summary')", () => {
    expect(classifyUserMessage("check inventory stock levels")).toBe(
      "inventory_summary",
    );
  });
...
describe("checkEscalation", () => {
  ...
  it("returns proceed for confidence >= 0.9", () => {
    const result = checkEscalation({ result: makeResult(0.95) });
    expect(result.action).toBe("proceed");
  });

  it("returns escalate_to_supervisor for confidence 0.7-0.89", () => {
    const result = checkEscalation({ result: makeResult(0.8) });
    expect(result.action).toBe("escalate_to_supervisor");
    expect(result.reason).toBeTruthy();
  });

  it("returns escalate_to_human for confidence < 0.7", () => {
    const result = checkEscalation({ result: makeResult(0.4) });
    expect(result.action).toBe("escalate_to_human");
  });

  it("handles edge case: exactly 0.7", () => {
    const result = checkEscalation({ result: makeResult(0.7) });
    expect(result.action).toBe("escalate_to_supervisor");
  });

  it("handles edge case: exactly 0.9", () => {
    const result = checkEscalation({ result: makeResult(0.9) });
    expect(result.action).toBe("proceed");
  });

  it("handles dollar threshold override", () => {
    const result = checkEscalation({
      result: makeResult(0.95),
  ...
  it("handles requiresHumanApproval flag", () => {
    const result = checkEscalation({
      result: makeResult(0.99),
  ...
  it("handles zero confidence", () => {
    const result = checkEscalation({ result: makeResult(0) });
    expect(result.action).toBe("escalate_to_human");
  });
});
