...
//
// Single implementation of the preset-pack install contract, used by the AI
// agent tool (install_tax_presets) AND surfaced to the web app (Settings →
// Taxes and the onboarding wizard re-export this from apps/web/server/lib),
// so the idempotency + version-continuation behavior can't drift between the
// two surfaces.
//
// Lives in the agents package (not the app) because the tool registry executes
// here; the app routers import it via `@xenboox/agents`.

import { and, eq, sql } from "drizzle-orm";
import { jurisdictionTaxRules } from "@xenboox/db/schema/tax-compliance";
import type { TaxPreset } from "./tax-presets";
