...
describe("AGENT_REGISTRY", () => {
  ...
  it("has exactly 1 tier1 agent (CFO)", () => {
    const tier1 = Object.values(AGENT_REGISTRY).filter(
      (a) => a.tier === "tier1",
    );
  ...
  it("has at least 4 tier2 agents", () => {
    const tier2 = Object.values(AGENT_REGISTRY).filter(
      (a) => a.tier === "tier2",
    );
  ...
  it("has tier3 and platform agents", () => {
    const tier3 = Object.values(AGENT_REGISTRY).filter(
      (a) => a.tier === "tier3",
    );
    const platform = Object.values(AGENT_REGISTRY).filter(
      (a) => a.tier === "platform",
    );
...
describe("checkEscalation — boundary values", () => {
  ...
  const boundaries = [
    ...
    { confidence: 0.69, expected: "escalate_to_human" },
    { confidence: 0.7, expected: "escalate_to_supervisor" },
    { confidence: 0.8, expected: "escalate_to_supervisor" },
    { confidence: 0.89, expected: "escalate_to_supervisor" },
  ...
  for (const { confidence, expected } of boundaries) {
    it(`confidence=${confidence} → ${expected}`, () => {
      expect(checkEscalation({ result: makeResult(confidence) }).action).toBe(
        expected,
      );
    });
  }
