...
export async function runConsolidationPipeline(
  ...
  try {
    ...
    // Also check for tags on subsidiary entries
  ...
  try {
    ...
    for (const [, pair] of matchedPairs) {
      if (pair.length < 2) continue; // Need both sides for elimination
      ...
      // Create elimination entry in consolidation layer
      const [ee] = await db
    ...
    if (runId) {
      await db
        ...
        .set({
          eliminationCount: elims.length,
          eliminationAmount: totalElimAmount.toFixed(2),
        })
    ...
    step3.result = {
      eliminationsCreated: elims.length,
      totalEliminationAmount: totalElimAmount,
