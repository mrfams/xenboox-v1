...
export async function nodeParseInput(state: ReportingStateType) {
  ...
  await trace.update({ output: { reportType: request.type, period: request.period, format: request.format } })
...
export async function nodeGenerateProfitLoss(state: ReportingStateType) {
  ...
  if (!periodId) {
    return { errors: ["Missing period for profit & loss report"], confidence: 0 }
  }
  ...
  try {
    ...
    await span.update({
      output: { revenue: pnl.revenue, expenses: pnl.expenses, netProfit: pnl.netProfit },
    })

    return {
      reportData: { profitAndLoss: pnl, balanceSheet: null, trialBalance: null },
      confidence: 0.92,
...
export async function nodeGenerateBalanceSheet(state: ReportingStateType) {
  ...
  try {
    ...
    await span.update({
      output: { assets: bs.assets, liabilities: bs.liabilities, equity: bs.equity },
    })

    return {
      reportData: { profitAndLoss: null, balanceSheet: bs, trialBalance: null },
      confidence: 0.92,
...
export async function nodeGenerateTrialBalance(state: ReportingStateType) {
  ...
  const periodId = state.currentRequest?.period ?? ""
  if (!periodId) {
    return { errors: ["Missing period for trial balance report"], confidence: 0 }
  }
  ...
  try {
    ...
    if (!tb.balanced) {
      return {
        reportData: { profitAndLoss: null, balanceSheet: null, trialBalance: tb },
        confidence: 0.0,
...
export async function nodeGenerateNarrative(state: ReportingStateType) {
  ...
  const narrative = generateNarrative(state.entityName, reportData, state.currency)
