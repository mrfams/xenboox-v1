...
export async function getFinancialRatios(entityId: string): Promise<FinancialRatio[]> {
  ...
  const totalAssets = assetAccounts.reduce((s, a) => s + 0, 0)
  const totalLiabilities = liabilityAccounts.reduce((s, a) => s + 0, 0)
  const totalEquity = equityAccounts.reduce((s, a) => s + 0, 0)
  const totalRevenue = revenueAccounts.reduce((s, a) => s + 0, 0)
  const totalExpenses = expenseAccounts.reduce((s, a) => s + 0, 0)
  ...
  const ratios: FinancialRatio[] = [
    {
      ...
      benchmark: "> 1.5",
      status: totalLiabilities > 0 && totalAssets / totalLiabilities >= 1.5 ? "good" : "warning",
    },
    {
      name: "Debt-to-Equity",
      ...
      benchmark: "< 2.0",
      status: totalEquity > 0 && totalLiabilities / totalEquity < 2.0 ? "good" : "warning",
    },
    {
      ...
      benchmark: "> 10%",
      status: totalRevenue > 0 && ((totalRevenue - totalExpenses) / totalRevenue) * 100 >= 10 ? "good" : "warning",
    },
    {
      name: "Return on Assets",
      value: totalAssets > 0 ? ((totalRevenue - totalExpenses) / totalAssets) * 100 : 0,
...
export async function getKpiDashboard(entityId: string): Promise<AnalyticsSummary> {
  const ratios = await getFinancialRatios(entityId)

  const insights: string[] = []

  if (ratios.find((r) => r.name === "Current Ratio" && r.status === "warning")) {
    insights.push("Current ratio below benchmark — review liquidity position")
  }
  if (ratios.find((r) => r.name === "Net Profit Margin" && r.status === "warning")) {
...
export async function getTrendAnalysis(entityId: string, periods: number = 6): Promise<AnalyticsSummary> {
  const ratios = await getFinancialRatios(entityId)
...
export async function getCashFlowAnalysis(entityId: string): Promise<AnalyticsSummary> {
  const ratios = await getFinancialRatios(entityId)

  return {
    period: new Date().toISOString().slice(0, 7),
    ratios: ratios.filter((r) => ["Current Ratio", "Working Capital"].includes(r.name)),
    insights: ["Cash flow analysis requires bank and cash account transaction data"],
    kpis: {
      operatingCashFlow: 0,
      investingCashFlow: 0,
      financingCashFlow: 0,
