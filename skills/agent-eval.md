...
> Use this when creating and running agent evaluation tests against the golden dataset.
...
export const invoiceCategorizationTests: TestScenario[] = [
  {
    id: "inv-cat-001",
    agentId: "ap-agent",
    description: "Office supplies invoice should be categorized to office supplies expense",
    category: "invoice_categorization",
    input: {
      entityId: "test-entity-gambia",
      taskType: "categorize_invoice",
      data: {
        vendor: "Hardware Store Ltd",
        items: [
          { description: "A4 Paper Ream", amount: 250 },
          { description: "Printer Toner", amount: 450 }
        ],
        total: 700,
        currency: "GMD"
      }
    ...
    expected: {
      result: {
        accountId: "5400",  // Office Supplies expense
        confidence: { gte: 0.8 }
      },
      minConfidence: 0.8,
      shouldEscalate: false,
      expectedActions: ["categorize", "post_journal_entry"]
    },
    tolerance: { amount: 0.01, percentage: 0 }
  },

  {
    id: "inv-cat-002",
    agentId: "ap-agent",
    description: "Ambiguous vendor should escalate for human review",
    category: "invoice_categorization",
    input: {
      entityId: "test-entity-gambia",
      taskType: "categorize_invoice",
      data: {
        vendor: "ABC Company",
        items: [
          { description: "General items", amount: 5000 }
        ],
        total: 5000,
        currency: "GMD"
      }
    ...
    expected: {
      ...
      minConfidence: 0.4,
      shouldEscalate: true,
      expectedActions: ["categorize", "escalate"]
    },
    tolerance: { amount: 0, percentage: 0 }
  }
]

// ─── EXAMPLE: Journal Entry Validation ───────────────

export const journalEntryTests: TestScenario[] = [
  {
    id: "je-001",
    agentId: "ledger-agent",
    description: "Balanced journal entry should be posted successfully",
    category: "journal_entry",
    input: {
      entityId: "test-entity-gambia",
      taskType: "validate_and_post",
      data: {
        description: "Purchase of office supplies",
        entries: [
          { accountId: "5400", debit: 1000, credit: 0 },
    ...
    expected: {
      ...
      minConfidence: 1.0,
      shouldEscalate: false,
      expectedActions: ["validate", "check_double_entry", "post"]
    },
    tolerance: { amount: 0.01, percentage: 0 }
  },

  {
    id: "je-002",
    agentId: "ledger-agent",
    description: "Unbalanced journal entry should be rejected",
    category: "journal_entry",
    input: {
      entityId: "test-entity-gambia",
      taskType: "validate_and_post",
    ...
    expected: {
      result: { posted: false, error: "Debits do not equal credits" },
      minConfidence: 1.0,
      shouldEscalate: false,
      expectedActions: ["validate", "check_double_entry", "reject"]
    },
    tolerance: { amount: 0, percentage: 0 }
  }
]
...
export async function runEvaluation(
  ...
  for (const scenario of scenarios) {
    ...
    try {
      ...
      // Compare results
      const confidenceCheck = output.confidence >= scenario.expected.minConfidence
      ...
      const passed = confidenceCheck && actionsCheck &&
      ...
      results.push({
        scenarioId: scenario.id,
        passed: false,
        confidence: { actual: 0, expected: scenario.expected.minConfidence, passed: false },
        actions: { actual: [], expected: scenario.expected.expectedActions, passed: false },
        result: { actual: null, expected: scenario.expected.result, passed: false },
        durationMs: Date.now() - startTime,
...
export function generateReport(
  ...
  return {
    ...
    averageConfidence: results.reduce((sum, r) => sum + r.confidence.actual, 0) / results.length,
