...
- `[x]` Audit current dependencies for known vulnerabilities — **DONE (Aug 16, 2026): full `pnpm audit --prod` remediation. 66 → 24 findings. All production-runtime HIGHs fixed: `drizzle-orm` 0.44.2 → 0.45.2 (SQL-identifier injection) across 6 packages, `socket.io` chain (socket.io 4.8.3 / engine.io 6.6.9 / ws 8.21.0 / socket.io-parser 4.2.7 / cookie 0.7.2 via @trigger.dev), `sharp` → 0.35.3 (libvips CVEs), `langsmith` → 0.6.3 (public prompt-pull deserialization), `react-router` → 7.18.2, `postcss` → 8.5.26, plus fast-uri/brace-expansion/js-yaml/undici/nanoid/@opentelemetry-core/prismjs overrides. Zero test regressions (web 1025 pass/16 pre-existing fail before & after; agent suite 197 pre-existing pipeline failures identical before & after). Remaining 5 findings are Expo-CLI mobile build tooling (never deployed) + 2 major-version-jump deferrals — see §22.1.**
- `[x]` Lock file integrity — verify `pnpm-lock.yaml` is committed and not tampered — **`pnpm-lock.yaml` tracked in git; `pnpm install --frozen-lockfile` enforced in CI; regenerated lock committed with this audit pass (Aug 16, 2026)**
...
- `[x]` No real-time agent status dashboard for users (agent-monitor page exists but needs verification) — **DONE (Aug 16, 2026): `/dashboard/agent-monitor` is now a fully entity-scoped live surface. `liveRuns.getActiveEntityRuns` (queued/in_progress/waiting for the caller's entity) + `liveRuns.getEntityRunDetail` (steps + event log; foreign runIds resolve to null) feed a real-time page — SSE live events (Redis-backed, §16.1) merged with a 10s DB-poll backstop per §16.2 degraded modes, recent-executions table, 30-day per-entity cost/success header, and a run-detail drawer. Every query is entity-scoped by construction.**
...
- `[x]` Edge runtime only for middleware/redirects/geolocation — never DB or heavy crypto in middleware. — **verified: `middleware.ts` uses `edgeAuth` + `edgeAdminAuth` (edge-compatible), no DB queries, no heavy crypto; only auth/rate-limit/security-headers/redirects** (Aug 15, 2026)
...
- `[x]` Idle session timeout (configurable per plan; default 30–60 min for financial apps). — **`lib/auth/idle-session.ts`: `applyIdleTimeout()` stamps `lastActivity` into the JWT (throttled to once/60s so per-request Set-Cookie doesn't defeat CDN caching) and returns `null` once the window elapses — wired into BOTH main auth (`lib/auth/index.ts`) and admin auth (`lib/auth/admin.ts`) jwt callbacks, so a `null` return invalidates the session. Window configurable via `AUTH_IDLE_TIMEOUT_MINUTES` (default 60) — documented in `.env.example`. 6 unit tests + all 29 existing auth tests pass (Aug 16, 2026)**
...
- `[x]` RBAC is enforced server-side on every mutation (verified `rlsProtectedProcedure`) — add an automated test that enumerates role→procedure permissions (admin/accountant/viewer) and fails on drift. — _*`__tests__/rbac-sweep.test.ts`: static sweep of all 77 router files — (1) every query/mutation must be chained on a protected procedure (rlsProtected/rlsMutate/protected/mutate/admin*/auth/concurrencyLimited/planAware), with publicProcedure allowed only for the explicit pre-auth auth-flow keys (mfaChallenge, login, resetPassword, accept, health, …); (2) `requireRole` gates reference only known roles (owner/admin/finance_director/accountant/manager/viewer) and canonical owner-only + viewer-tier gates exist; (3) every router file must be mounted in `_app.ts`. Sweep caught and fixed: `workflowBuilderRouter` was never mounted (dead admin router — now mounted). 4 tests (Aug 16, 2026)_\*
- `[x]` Approval workflows (AP approval, close center, tax review) must validate workflow state transitions server-side to prevent race conditions/double-approval. — **all three approval surfaces now use atomic status-guarded transitions (Aug 16, 2026): (1) `approvals.resolve` (journal entries) — conditional `UPDATE … WHERE status='draft'`; the losing racer affects 0 rows → CONFLICT, so double-approve / approve-after-reject / reject-after-approve are impossible; agent escalations guarded via `humanResponse IS NULL` claim + pending-count now excludes resolved items. (2) `closeCenter.updateTaskStatus` — `WHERE status != input.status`; double-complete is an idempotent no-op with no duplicate audit row. (3) `ap.approvePO` — already had `WHERE status='draft'`; now throws CONFLICT on 0 rows instead of silently returning undefined. 7 race-condition tests in `__tests__/approval-race.test.ts` drive the real routers through createCaller and pin every race outcome.**
...
- `[x]` **SLOs:** availability 99.9% (SLA already promises it — must be measurable), p95 < 300ms on core tRPC reads, p99 < 1s on writes, error budget burn alerts. — **`docs/SLOs.md`: SLO table (availability 99.9%/30d via health probes, read p95<300ms, write p99<1s, error rate <0.1%, LLM cost anomaly), error-budget math (43.2 min/mo, burn-rate rules + deploy freeze at exhaustion), burn-alert thresholds (latency/error/availability/LLM spend) with actions, per-layer latency budget, measurement via existing OTel tRPC spans** (Aug 16, 2026)
- `[x]` **Health endpoints:** `/api/health?check=live` (process) and `/api/health?check=ready` (DB + Redis reachable) — upgraded to production-grade with real Redis ping, HTTP 503 on unhealthy, uptime tracking, and Anthropic API reachability check (§2.4). **Live-verified on xenboox.vercel.app (Aug 16, 2026): `?check=live` → 200 `healthy`, `?check=ready` → 200; note the paths are query-param form (the literal `/api/health/live` path 404s — probes in `docs/UPTIME_PROBES.md` already use the correct form).**
